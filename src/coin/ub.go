...
func (agent *UBAgent) GetRawTransactionRPC(trxid string) (string, error) {
	res, err := agent.DoHttpJsonRpcCallType1("getrawtransaction", trxid)
	if err != nil {
		return "", err
	}
	if res.Error != nil {
...
func UBGetRedeemScriptByPubKeys(needCount int, pubKeyStrList []string) (string, error) {
	if needCount <= 0 || needCount > 16 {
		return "", errors.New("UBGetRedeemScriptByPubKeys error: invalid needCount")
	}
	if len(pubKeyStrList) == 0 || len(pubKeyStrList) > 16 {
		return "", errors.New("UBGetRedeemScriptByPubKeys error: invalid pubKeyStrList size")
	}
	if needCount > len(pubKeyStrList) {
		return "", errors.New("UBGetRedeemScriptByPubKeys error: needCount greater than pubKeyStrList size")
	}
	...
	for _, pubKeyStr := range pubKeyStrList {
		...
		pubKeyCpsBytes, err := UBGetCompressPubKey(pubKeyBytes)
		if err != nil {
			return "", err
		}
...
func UBGetMultiSignAddressByRedeemScript(redeemScriptStr string) (string, error) {
...
func UBGetMultiSignScriptByRedeemScript(redeemScriptStr string) ([]byte, error) {
	redeemScript, err := hex.DecodeString(redeemScriptStr)
	if err != nil {
		return nil, err
	}
...
func (agent *UBAgent) BuildTrxFromUtxosRPC(utxos []UTXODetail, addrFromStr string, addrToStr string, amountTransferStr string, feeRateStr string) (string, InPuts, OutPuts, error) {
	supportCoin, ok := config.GlobalSupportCoinMgr[agent.CoinType()]
	if !ok {
		return "0", nil, nil, errors.New("not support coin")
	}
...
func UBCombineSignatureAndRedeemScript(signature []byte, redeemScriptBytes []byte) ([]byte, error) {
	bytesBuf := bytes.NewBuffer([]byte{})
	bufWriter := io.Writer(bytesBuf)
...
func (agent *UBAgent) SignRawTransactionRPC(rawTrx string, pubKeyStr string, keyIndex uint16, utxos []UTXODetail) (string, error) {
	pubKeyBytes, err := hex.DecodeString(pubKeyStr)
	if err != nil {
		return "", err
	}
...
func (agent *UBAgent) MultiSignRawTransactionRPC(rawTrx string, redeemScriptStr string, keyIndex uint16, utxos []UTXODetail) (string, error) {
	...
	for i := 0; i < len(trx.Vin); i++ {
		...
		scriptSig, err := UBCombineSignatureAndRedeemScript(signedData, redeemScriptBytes)
		if err != nil {
			return "", err
		}
		signedDataList[i] = scriptSig
...
func (agent *UBAgent) MultiVerifySignRawTransactionRPC(signedRawTrx string, pubKeyStr string, utxos []UTXODetail) error {
	trx, err := UBUnPackRawTransaction(signedRawTrx)
	if err != nil {
		return err
	}
	...
	for i := 0; i < len(trx.Vin); i++ {
		trxTemp, err := UBUnPackRawTransaction(signedRawTrx)
		if err != nil {
			return err
		}
		// set empty script
		...
		if signatureScriptBytes[len(signatureScriptBytes)-1] != 0x1|0x8 {
			return errors.New("invalid signature, not ended with 0x1[ALL|FORKID]")
		}
		...
		compressPubKey, err := UBGetCompressPubKey(pubKeyBytes)
		if err != nil {
			return err
		}
...
func (agent *UBAgent) CombineRawTransactionRPC(signedRawTrxs []string) (string, error) {
