...
func (agent *BTCAgent) GetRawTransactionRPC(trxid string) (string, error) {
...
func BTCGetMultiSignScriptByRedeemScript(redeemScriptStr string) ([]byte, error) {
	redeemScript, err := hex.DecodeString(redeemScriptStr)
	if err != nil {
		return nil, err
	}
...
func (agent *BTCAgent) BuildTrxFromUtxosRPC(utxos []UTXODetail, addrFromStr string, addrToStr string, amountTransferStr string, feeRateStr string) (string, InPuts, OutPuts, error) {
	supportCoin, ok := config.GlobalSupportCoinMgr[agent.CoinType()]
	if !ok {
		return "0", nil, nil, errors.New("not support coin")
	}
...
func (agent *BTCAgent) SignRawTransactionRPC(rawTrx string, pubKeyStr string, keyIndex uint16, utxos []UTXODetail) (string, error) {
	pubKeyBytes, err := hex.DecodeString(pubKeyStr)
	if err != nil {
		return "", err
	}
...
func (agent *BTCAgent) MultiSignRawTransactionRPC(rawTrx string, redeemScriptStr string, keyIndex uint16, utxos []UTXODetail) (string, error) {
...
func (agent *BTCAgent) MultiVerifySignRawTransactionRPC(signedRawTrx string, pubKeyStr string, utxos []UTXODetail) error {
	trx, err := BTCUnPackRawTransaction(signedRawTrx)
	if err != nil {
		return err
	}
	...
	for i := 0; i < len(trx.Vin); i++ {
		trxTemp, err := BTCUnPackRawTransaction(signedRawTrx)
		if err != nil {
			return err
		}
		// set empty script
		...
		compressPubKey, err := BTCGetCompressPubKey(pubKeyBytes)
		if err != nil {
			return err
		}
...
func (agent *BTCAgent) CombineRawTransactionRPC(signedRawTrxs []string) (string, error) {
